...
export const ContextMenuItem: React.FC<React.ButtonHTMLAttributes<HTMLButtonElement>> = ({ className = '', ...props }) => (
    <button
      className={`block w-full text-left px-4 py-2 text-sm font-body text-neutral-400 hover:bg-neutral-900 hover:text-soul-accent transition-colors ${className}`}
      {...props}
    />
);
...
export const ContextMenuLabel: React.FC<{ children: React.ReactNode }> = ({ children }) => (
    <div className="px-4 py-1 text-xs font-display  tracking-normal text-neutral-600">
        {children}
    </div>
);
