...
export const SidebarHeader = React.memo<{ children: React.ReactNode; className?: string }>(({
  ...
  <div className={`h-14 flex items-center px-4 border-b border-neutral-800 font-display  tracking-normal text-sm text-soul-accent whitespace-nowrap overflow-hidden ${className}`}>
    {children}
  </div>
));
...
export const SidebarItem = React.memo<SidebarItemProps>(({
  ...
  return (
    <button
      ...
      className={`
        w-full flex items-center gap-3 px-3 py-2 text-sm font-body transition-colors relative group
