  ...
  return (
    <div className="relative" ref={containerRef}>
      <button 
        ...
        className={`px-3 py-1 text-sm font-display  tracking-normal transition-colors rounded-sm ${isOpen ? 'bg-neutral-900 text-neutral-200' : 'text-neutral-500 hover:text-neutral-300 hover:bg-neutral-900/50'}`}
      >
        {trigger}
      </button>
      {isOpen && (
...
export const MenubarItem: React.FC<{ children: React.ReactNode; shortcut?: string; onClick?: () => void }> = ({ children, shortcut, onClick }) => (
    <button 
        onClick={onClick}
        className="w-full text-left px-4 py-1.5 text-sm font-body text-neutral-400 hover:bg-neutral-900 hover:text-soul-accent transition-colors flex justify-between items-center group"
    >
        <span>{children}</span>
